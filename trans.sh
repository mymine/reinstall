...
add_fallback_efi_to_nvram() {
    ...
    if efi_row=$(lsblk /dev/$xda -ro NAME,PARTTYPE,PARTUUID | grep -i "$EFI_UUID"); then
        ...
        cat <<EOF
Warning: This machine uses EFI boot, but the main hard drive does not have an EFI partition.
If this machine supports Legacy BIOS boot (CSM), you can run the reboot command to restart.
Otherwise, the system may be unbootable after restarting.

警告：本机使用 EFI 引导，但主硬盘没有 EFI 分区。
如果本机支持 Legacy BIOS 引导 (CSM)，你可以运行 reboot 命令重启。
否则重启后可能无法进入系统。
EOF
        exit
    fi
